...
## Phase 3-experimental: learned class-affinity ambiguity as a CS-SupCon training signal (research only, `feature/learned-ambiguity`)

**IMPLEMENTATION AND TESTS ONLY - NO TRAINING WAS RUN.** Following Phase 3b's finding that Phase 3 ambiguity did not clearly add error-detection information beyond EDL uncertainty (combined - EDL AUROC = -0.0082574917, bootstrap 95% CI [-0.0444923, 0.0270816] - crossing zero), this phase does not attempt a different score fusion. Instead it tests a different hypothesis: whether the Phase 3 learned class-affinity *structure* is useful as a CS-SupCon training signal, replacing the manually-defined hard pairs in a controlled experiment.

### Discovery: the CS-SupCon mechanism for this already existed

Before writing any new loss code, `src/losses/cs_supcon.py` and `src/losses/ambiguity.py` were inspected (per the explicit instruction to reuse an existing scale parameter rather than inventing one). This found `ambiguity_source="learned_class"` (Phase 1, already implemented and already wired end-to-end into `src/training/run_aa_evidentnet.py`): a generic mechanism that installs ANY frozen `[K,K]` matrix via `CSSupConLoss.set_learned_ambiguity_matrix(...)` and applies `w(i,a) = 1 + ambiguity_scale * A[y_i, y_a]` for negatives - the exact equation this task specified. `ambiguity_scale` (default `DEFAULT_AMBIGUITY_SCALE = 1.0`, `src/losses/ambiguity.py`) already existed as a predetermined, shared config field (`configs/losses.yaml: cs_supcon.ambiguity_scale`), documented as chosen so a maximally-ambiguous pair (A[a,b]=1) receives the same weight (2.0) as the existing fixed `ambiguity_weight` default - i.e. already comparable in magnitude to the existing mechanism, not arbitrary.

**Conclusion: no new numeric hyperparameter was introduced.** `ambiguity_scale=1.0` is reused UNCHANGED for the new mode - this satisfies the explicit instruction to reuse a predetermined value rather than choose/tune one.

### What was actually added

CS-SupCon's `forward()` math is completely unmodified (same formula, same masking, same log-sum-exp stability) - only its *dispatch* was extended:

1. `src/losses/ambiguity.py`: `VALID_AMBIGUITY_SOURCES` gains a third value, `"learned_class_affinity"`; `load_ambiguity_settings` validates it identically to `"learned_class"` (requires `ambiguity_scale > 0` and `reference_checkpoint_path`).
2. `src/losses/cs_supcon.py`: `CSSupConLoss.__init__`'s valid-source check and `forward()`'s branch condition both become `in ("learned_class", "learned_class_affinity")` - the SAME code executes for either value; `"fixed_pairs"`'s branch is untouched (byte-identical), and its existing tests were re-run unmodified to confirm.
3. `src/training/class_affinity_ambiguity_cs_supcon_setup.py` (new): `build_class_affinity_ambiguity_for_cs_supcon(...)` - a thin wrapper that calls the EXISTING, completely unmodified `src.training.class_affinity_ambiguity_setup.build_class_affinity_ambiguity` (Phase 3's own matrix construction: `m=5`, `temperature=0.1`, both predetermined before Phase 3's validation ever ran) and converts its `matrix_numpy` to a frozen buffer via the EXISTING `src.losses.ambiguity.class_ambiguity_matrix_to_buffer` (already generic to any `[K,K]` matrix - not Phase-1-specific despite living in that module). No equation from Phase 3 was re-derived.
4. `src/training/run_aa_evidentnet.py`: the existing `ambiguity_source == "learned_class"` dispatch block is generalized to `in ("learned_class", "learned_class_affinity")`, calling `build_class_affinity_ambiguity_for_cs_supcon` instead of `build_learned_class_ambiguity` for the new value; both install their matrix via the IDENTICAL `criterion.cs_supcon_loss.set_learned_ambiguity_matrix(...)` call. Mode-specific metadata fields (Phase 1's `class_sample_counts`/`margin_normalization` vs. Phase 3's `m`/`temperature`/`scale_percentile`/`matrix_construction_method`) are kept separate so neither mode's metadata is polluted with the other's fields. The registry `notes` column is tagged `ambiguity_source=<value>` whenever it is not the default `"fixed_pairs"`. A CSV export of the matrix (`ambiguity_class_matrix.csv`, alongside the existing `ambiguity_metadata.json`) was added for BOTH modes (additive - `"learned_class"` runs now also get this CSV, a strictly backward-compatible addition since no prior test asserted its absence).

### Frozen-matrix guarantees

Identical to Phase 1's `"learned_class"` mechanism (unchanged): the reference checkpoint is loaded into its own throwaway `create_model(...)` instance, `eval()`, every parameter's `requires_grad` forced `False`, used only under `torch.inference_mode()` (all inherited unmodified from `build_class_affinity_ambiguity`); the resulting matrix is registered as a `torch` buffer (`requires_grad=False`) on `CSSupConLoss`, built exactly once before `optimizer`/`scheduler`/`Trainer` are constructed, and never recomputed or touched by an optimizer/backward pass afterward. The experimental model itself is a fresh `create_model(...)` call, identical to every other `run_aa_evidentnet_training` invocation - never initialized from the reference checkpoint's weights (verified by `test_experimental_model_is_freshly_initialized_not_loaded_from_reference_checkpoint`, which confirms two independent model instances with non-overlapping parameter storage).

### Leakage safeguards

`build_class_affinity_ambiguity_for_cs_supcon` has no test-manifest parameter (checked structurally); reads `train_original.csv` only, proven unaffected by a `test_original.csv`'s mere presence in the same directory; raises clearly on a missing `train_original.csv`. `run_aa_evidentnet_training`'s existing test-set lock (no test-manifest anywhere in that function) is unmodified and applies identically to this new mode.

### Known open issue - must be resolved before any real training run

The reference checkpoint specified for the real experiment, `checkpoints/20260831_064112_aa_evidentnet_seed42_48c214/best.pt` (expected SHA-256 `800d6184416d77a2a2d1447680ca7869bb973d62fc361f15a6ddfa0e95be2c2a`), **does not exist in this local environment** - only four older, unrelated checkpoints exist under `results/checkpoints/` (`20260827_093914_maxvit_seed42_65f791`, `20260827_150813_aa_evidentnet_seed42_393841`, `20260827_152101_aa_evidentnet_seed42_c4f592`, `20260827_153917_resnet50_seed42_5f8341`). This module has no built-in mechanism to assert an installed checkpoint's SHA-256 against an "expected" value (consistent with every other setup module in this project, which record the ACTUAL hash used but do not verify it against an external expectation) - the checkpoint's presence and hash must be independently verified (e.g. on Colab, where the file actually resides) before the real experimental run, not assumed.

### Tests added

`tests/test_ambiguity.py` (+4: `learned_class_affinity` requires `reference_checkpoint_path`, rejects non-positive `ambiguity_scale`, accepts a valid config, reuses the same default scale as `learned_class`; 1 existing test updated from "exactly two" to "exactly three" valid sources - a necessary, intentional change, not a weakening). `tests/test_cs_supcon.py` (+10: matrix required before `forward()`; matrix installed as a frozen buffer with defensive cloning; the exact `1 + ambiguity_scale * A[y_i,y_a]` formula verified against an INDEPENDENT reference computation built by direct matrix indexing, never by calling into `CSSupConLoss`'s own internals; a zero matrix reproduces the uniform `fixed_pairs`-with-no-pairs baseline exactly; a strictly larger matrix entry produces a strictly larger loss on a 3-sample fixture isolating that one pair; matrix-must-be-square validation; `"learned_class"` and `"learned_class_affinity"` produce IDENTICAL loss values given the same installed matrix; unknown `ambiguity_source` still rejected). `tests/test_class_affinity_ambiguity_cs_supcon_setup.py` (15, new module: matrix matches the unmodified Phase 3 setup function exactly; installable into `CSSupConLoss` with a real backward pass; symmetric/zero-diagonal/`[0,1]`-bounded; frozen/non-trainable buffer, absent from `.parameters()`; train-original-only construction unaffected by a `test_original.csv`'s presence; no test-manifest parameter; missing-manifest failure; deterministic across repeated calls; no epoch/step-shaped parameter; reference model verified `eval()`+`requires_grad=False`; reference checkpoint file hash unchanged after use; full provenance recorded on the artifact). `tests/test_run_aa_evidentnet_ambiguity.py` (+6, mirroring every existing `"learned_class"` integration test for `"learned_class_affinity"`: matrix built before `Trainer.fit()` and installed, Phase 1's builder never called, registry tagged, mode-specific metadata fields present and the OTHER mode's fields absent; smoke-test incompatibility; missing `reference_checkpoint_path`; missing `train_original.csv`; incompatible reference checkpoint; fresh independent model initialization).

### Full test-suite result

All new/modified test files re-run individually plus the full repository suite - see the assistant's final report for this change for exact pass counts. No test was weakened to make it pass; the one intentionally-updated pre-existing test (`test_valid_ambiguity_sources_are_exactly_two` -> `..._exactly_three`) reflects a real, deliberate change to the valid-values tuple, not a loosened assertion.
