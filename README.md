...
auto-resolve.** `python run_pipeline.py audit` fails (exit code 1) by
default because of it (see `configs/dataset.yaml: audit.policies`). Full
details are in `data/audit/duplicate_report.csv` and
`data/audit/leakage_report.csv` (both regenerated by re-running the audit;
not committed to git). Resolving which label (if any) is authoritative for
each duplicated image is a prerequisite for Task 3 (splitting) — an image
in a cross-class duplicate group must not end up in different classes
across train/val/test, and arguably should not be used with an unresolved
contradictory label at all. This decision requires human review and must
not be made automatically by future pipeline code.
