"""Phase 2: validation-only analysis of the neighborhood-based ambiguity
signal, and a direct comparison against Phase 1's prototype-based
ambiguity (src/evaluation/ambiguity_validation.py, left completely
unmodified and intact).

Answers the SAME question Phase 1's validation module asks - "is this
ambiguity signal actually meaningful?" - using ONLY
data/manifests/val_original.csv and the SAME frozen reference
representation (train embeddings/labels, neighborhood matrix) already
built by
src.training.neighborhood_ambiguity_setup.build_neighborhood_class_ambiguity
from train_original.csv. This module never reads, requires, or depends on
data/manifests/test_original.csv in any way.

Nine analyses, directly paralleling Phase 1's seven (see
src/evaluation/ambiguity_validation.py's own docstring) plus two
neighborhood-specific additions:

  1-8. Identical in kind to Phase 1's analyses 1-7 (error-detection
       AUROC/AUPRC; competing-class hit rate among errors; ambiguity
       mean/median for correct vs. incorrect; ambiguity for the existing
       hard-pair classes; Spearman vs. EDL uncertainty; ambiguity/EDL
       quadrant error rates; learned-matrix-vs-confusion-matrix ranking
       comparison), computed from `ambiguity_margin` (the PRIMARY
       neighborhood score - see src/losses/neighborhood_ambiguity.py)
       instead of Phase 1's prototype-margin score.
  9. Top-N neighborhood-ambiguity class pairs (reusing
     src.evaluation.ambiguity_validation's own `_offdiagonal_pairs`/
     `_top_n_pairs` helpers, same-package cross-import).

A separate function, `build_prototype_vs_neighborhood_comparison`, reads
Phase 1's OWN saved `validation_metrics.json`/`class_ambiguity_matrix.csv`
artifacts (produced by a prior, separate `run_ambiguity_validation` call -
this module does not recompute Phase 1's results, and does not require
Phase 1 to have been run in the same process) and produces a side-by-side
comparison, including where the three existing fixed hard pairs
(Healthy/Glaucoma, Disc Edema/Glaucoma, Diabetic Retinopathy/Central
Serous Chorioretinopathy) rank in each matrix. This function makes NO
claim about which method is "better" - it only reports the numbers side
by side; that judgment is left to whoever reviews the comparison.
"""
...
from src.losses.neighborhood_ambiguity import compute_validation_neighborhood_ambiguity
...
from src.training.neighborhood_ambiguity_setup import NeighborhoodAmbiguityArtifact
...
class NeighborhoodAmbiguityValidationError(Exception):
    """Raised for a problem running validation-only neighborhood-ambiguity
    analysis: a missing val_original.csv, or an internal invariant
    violation."""


@dataclass
class NeighborhoodAmbiguityValidationSummary:
    run_id: str
    num_val_samples: int
    k: int
...
def run_neighborhood_ambiguity_validation(
    artifact: NeighborhoodAmbiguityArtifact,
...
) -> NeighborhoodAmbiguityValidationSummary:
    """Run the neighborhood-ambiguity validation analyses against
    `val_manifest_path` (data/manifests/val_original.csv) using the SAME
    reference checkpoint and frozen train embeddings/matrix already built
    by
    `src.training.neighborhood_ambiguity_setup.build_neighborhood_class_ambiguity`
    ...
    if not val_manifest_path.is_file():
        raise NeighborhoodAmbiguityValidationError(
    ...
    val_result = compute_validation_neighborhood_ambiguity(
        extracted.embeddings, artifact.train_embeddings, artifact.train_labels, num_classes, k=artifact.k
    )
    ambiguity = val_result.ambiguity_margin
    competing_class = val_result.competing_class
    ...
    spearman_entropy = _spearman(val_result.ambiguity_entropy.tolist(), edl_uncertainty.tolist())
    ...
    # --- 7. neighborhood matrix vs. observed val confusion (comparison only) ---
    ...
    # --- outputs ---
    run_id = generate_run_id("neighborhood_ambiguity_validation", seed=0, smoke_test=False)
    base_dir = Path(output_dir) / run_id
    base_dir.mkdir(parents=True, exist_ok=True)

    matrix_path = base_dir / "neighborhood_class_ambiguity_matrix.csv"
    ...
    summary = NeighborhoodAmbiguityValidationSummary(
        run_id=run_id,
        ...
        metrics_path=str(base_dir / "neighborhood_validation_metrics.json"),
        metadata_path=str(base_dir / "neighborhood_metadata.json"),
    )
    ...
    metadata = {
        "run_id": run_id,
        "method": "neighborhood_ambiguity (Phase 2) - analysis only, not wired into training",
        "k": artifact.k,
        "matrix_formula": (
            ...
            "A_sym = (score(a->b)+score(b->a))/2, diagonal=0; "
            "A = min-max rescale of A_sym's off-diagonal entries to [0,1] (train-derived only)"
        ),
        "sample_ambiguity_formula": (
            "top-k nearest TRAIN neighbors (any class) per validation embedding; "
            "p_c = neighbor-class fraction; margin = p_(1)-p_(2); "
            "ambiguity_margin (PRIMARY) = 1-margin; ambiguity_entropy (secondary) = H(p)/log(K)"
        ),
        "normalization_method": (
            "Class matrix: min-max over its own off-diagonal entries (train-derived). "
            "Sample validation ambiguity: none required - both diagnostics are proportions/"
            "probabilities, naturally bounded in [0,1] by construction."
        ),
...
def _load_matrix_csv(path: Union[str, Path], canonical_classes: Sequence[str]) -> np.ndarray:
    """Read back a class_ambiguity_matrix.csv (either Phase 1's or Phase
    2's - both share the same `class_name` + one-column-per-class schema,
    written by `write_csv`) into a [K, K] numpy array."""
    num_classes = len(canonical_classes)
...
def _pair_rank_and_value(matrix: np.ndarray, canonical_classes: Sequence[str], class_a: str, class_b: str) -> Tuple[Optional[int], Optional[float]]:
    """1-indexed rank (1 = most ambiguous pair) and raw value of
    (class_a, class_b) among all off-diagonal pairs of `matrix`. Returns
    (None, None) if either class name is not in `canonical_classes`."""
...
def build_prototype_vs_neighborhood_comparison(
    phase1_metrics_path: Union[str, Path],
    phase1_matrix_path: Union[str, Path],
    phase2_summary: NeighborhoodAmbiguityValidationSummary,
    phase2_matrix: np.ndarray,
    canonical_classes: Sequence[str],
    output_path: Union[str, Path],
    named_pairs: Optional[Sequence[Sequence[str]]] = None,
) -> Dict[str, Any]:
    """Read Phase 1's OWN saved artifacts (`phase1_metrics_path` =
    validation_metrics.json, `phase1_matrix_path` = class_ambiguity_matrix.csv,
    both produced by a prior `run_ambiguity_validation` call - never
    recomputed here) and build a side-by-side comparison against
    `phase2_summary`/`phase2_matrix`. Makes NO claim about which method is
    "better" - reports the numbers only.
    """
        ...
        named_pair_comparison.append(
            {
                ...
                "phase2_rank_improved": (
                    None if p1_rank is None or p2_rank is None else bool(p2_rank < p1_rank)
                ),
            }
        )
    ...
    comparison = {
        "note": (
            "This is a factual side-by-side comparison only. It does NOT claim the neighborhood "
            "method is better or worse than the prototype method - that judgment requires reviewing "
            "the actual numbers below, computed on val_original.csv only (never test_original.csv)."
        ),
        "phase1_prototype": {
            "error_detection_auroc": phase1_metrics.get("error_detection_auroc"),
            "error_detection_auprc": phase1_metrics.get("error_detection_auprc"),
            "class_matrix_vs_confusion_spearman": phase1_metrics.get("class_matrix_vs_confusion_spearman"),
            "competing_class_hit_rate_among_errors": phase1_metrics.get("competing_class_hit_rate_among_errors"),
        },
        "phase2_neighborhood": {
            "error_detection_auroc": phase2_summary.error_detection_auroc,
            "error_detection_auprc": phase2_summary.error_detection_auprc,
            "class_matrix_vs_confusion_spearman": phase2_summary.class_matrix_vs_confusion_spearman,
            "competing_class_hit_rate_among_errors": phase2_summary.competing_class_hit_rate_among_errors,
        },
